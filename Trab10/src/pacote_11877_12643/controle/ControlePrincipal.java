...
public class ControlePrincipal
{
	 ...
	 */
	private List<VirolaReta> listaVirolas;
	/**
	...
	public ControlePrincipal()
	{
		...
		listaVirolas = new ArrayList<VirolaReta>();
	...
	private void carregaVirolas()
	{
		...
		try
		{
			...
			for (int i = 1; i <= 3; i++)
			{
				...
				listaVirolas.add(new VirolaReta(new Ponto(0, 0), numeroArcosSuperior,
	...
	public void addFiguraOriginal(int x, int y, int tipo)
	{
		...
		figuraOriginal = new VirolaReta(new Ponto(x, y), listaVirolas.get(tipo));		
	...
	public void fazerCorteRetas(int xEsq, int yInf, int xDir, int ySup)
	{
		// Declaracao de variaveis.
		Stack<Figura> pilhaFiguras, pilhaCortada;
		Viewport janela;
		Figura novaFigura;
		
		// Inicializacao
		pilhaFiguras = pilhaPilhasFiguras.peek();
		pilhaCortada = new Stack<Figura>();
		janela = new Viewport(new Ponto(xEsq, yInf), new Ponto(xDir, ySup));

		// Varre a pilha de figuras, aplicando a transformacao.
		for (Figura figura : pilhaFiguras)		
		{
			novaFigura = figura.corteRetas(janela);
			pilhaCortada.add(novaFigura);
		}

		// Adiciona nova pilha.
		pilhaPilhasFiguras.push(pilhaCortada);
	}

	public void fazerCortePoligonos(int xEsq, int yInf, int xDir, int ySup)
	{
		// Declaracao de variaveis.
		Stack<Figura> pilhaFiguras, pilhaCortada;
		Viewport janela;
		...
		for (Figura figura : pilhaFiguras)		
		{
			novaFigura = figura.cortePoligonos(janela);
			pilhaCortada.add(novaFigura);
		}
