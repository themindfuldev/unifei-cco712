...
public class JanelaPrincipal extends JFrame implements ActionListener, MouseListener
{
	...
	// Componentes GUI
	private JLabel lblObjetos, lblCores;
	private JComboBox cmbObjetos, cmbCores;
	private JRadioButton rdoReferenciaOrigem, rdoOutroPontoReferencia,
			rdoTransformacaoSimples, rdoTransformacaoComposta;
	private JPanel pnlMenuSuperior, pnlMenuInferior, pnlReferencia,
			pnlTipoTransformacao;
	private PanelDesenho pnlAreaDesenho;
	private JButton btnReinicia, btnTranslada, btnRotaciona, btnEscala, btnUndo,
			btnFinalizaTransformacoes, btnFechaPrograma, btnCorteReta, btnCortePoligono;
	...
	public JanelaPrincipal(ControlePrincipal controle)
	{
		...
		// Corte
		btnCorteReta = new JButton("Corte retas");
		btnCortePoligono = new JButton("Corte poligonos");
		btnCortePoligono.addActionListener(this);
		btnCorteReta.addActionListener(this);
		...
		pnlMenuSuperior.add(btnCorteReta);
		pnlMenuSuperior.add(btnCortePoligono);
    ...
    */
	public void actionPerformed(ActionEvent e)
	{
		...
		if (source == btnCortePoligono)
		{
			optCortePoligono();
			return;
		}
		if (source == btnCorteReta)
		{
			optCorteRetas();
		}
	}

	private void optCortePoligono()
	{
//	 Declaracao de variaveis.
		...
		p1 = new Ponto(xEsq, yInf);
		p2 = new Ponto(xEsq, ySup);
		p3 = new Ponto(xDir, yInf);
		p4 = new Ponto(xDir, ySup);
		
		// Realiza operacao.
		controle.fazerCortePoligonos(xEsq, yInf, xDir, ySup);
		
		// atualizar lista de figuras exibida no panel
		pnlAreaDesenho.limpar();
		
		pnlAreaDesenho.adicionarFormaDesenhavel(new Segmento(p1, p2));
		pnlAreaDesenho.adicionarFormaDesenhavel(new Segmento(p2, p4));
		pnlAreaDesenho.adicionarFormaDesenhavel(new Segmento(p4, p3));
		pnlAreaDesenho.adicionarFormaDesenhavel(new Segmento(p3, p1));
	...
	private void optCorteRetas()
	{
		...
		// Realiza operacao.
		controle.fazerCorteRetas(xEsq, yInf, xDir, ySup);
